...
import random
import settings
from .tank import Tank  # Относительный импорт для пакета objects/

class EnemyTank(Tank):
    """БАЗОВЫЙ ВРАГ - твоя текущая логика + исправления"""
    ...
    def update(self, player, game):
        ...
        # Обновление кулдауна и пуль
        if self.shoot_cooldown > 0:
            self.shoot_cooldown -= 1
        self.bullets.update()
        self.rotate()

# Класс Node для будущего A* (пока не используется)
class Node:
